...
namespace Axon.Collections
{
	...
	: ICollection<PriorityValuePair<T>>
	{
		...
		/// </summary>
		private readonly List<PriorityValuePair<T>> _data;
		...
		{
			_data = new List<PriorityValuePair<T>>();
		}
		...
		ConcurrentBinaryMinHeap(int initialCapacity)
		{
			try
			{
				_data = new List<PriorityValuePair<T>>(initialCapacity);
			}
			catch (Exception e)
			{
				throw e;
		...
		Push(double priority, T value)
		{
			...
			{
				// Add a new element to the heap at the end of the data list.
				_data.Add(new PriorityValuePair<T>(priority, value));
		...
		Remove(PriorityValuePair<T> element)
		{
			...
			try
			{
				...
				else
				{
					...
					SwapElements(index, _data.Count - 1);
					_data.RemoveAt(_data.Count - 1);
		...
		SwapElements(int index1, int index2)
		{
			...
			try
			{
				...
				PriorityValuePair<T> temp = _data[index1];
		...
		HeapifyBottomUp(int index)
		{
			...
			try
			{
				...
				while (index > 0)
				{
					...
					priority = _data[index].Priority;
					priorityParent = _data[parentIndex].Priority;
		...
		HeapifyTopDown(int index)
		{
			...
			try
			{
				...
				while (true)
				{
					...
					double priority = _data[index].Priority;
					...
					if (rightIndex < _data.Count)
					{
						double priorityRightChild = _data[rightIndex].Priority;
						if (priority < priorityRightChild)
						{
							...
							priority = _data[rightIndex].Priority;
						}
					}
